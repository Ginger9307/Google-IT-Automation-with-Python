...
# 2. The show_letters function should print out each letter of a word on a separate line. 
...
# For example: 25 has 2 digits and 144 has 3 digits. Tip: you can figure out 
# the digits of a number by dividing it by 10 once per digit until there are no digits left.
...
# 4. This function prints out a multiplication table (where each number is the result 
...
# 6. The even_numbers function returns a space-separated string of all positive numbers
...
# Practice Quiz: Recursion 3
# Fill in the blanks to make the is_power_of function return whether the number 
# is a power of the given base. Note: base is assumed to be a positive number. 
...
# Practice Quiz: Recursion 4. 
# The count_users function recursively counts the amount of users that belong to a group in the company system, 
# by going through each of the members of a group and if one of them is a group, recursively calling 
# the function and counting the members.
...
def sum_positive_numbers(n):
  sum_pos = n
  if n > 0:
    sum_pos += sum_positive_numbers(n-1)
  else:
    return sum_pos
